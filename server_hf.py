...
def init_model():
    """初始化 Whisper 模型"""
    global MODEL, CC
    logger.info("正在从 HuggingFace 加载 Whisper 模型...")
    ...
    MODEL = pipeline(
        ...
        model="openai/whisper-medium",  # 或 whisper-small
        device="cpu",
...
async def handle_streaming_client(websocket, path):
    ...
    try:
        await websocket.send(json.dumps({
            "type": "connected",
            "message": "已连接到 HuggingFace Whisper STT 服务",
            "mode": "streaming"
        }))
...
async def main():
    ...
    logger.info(f"启动 HuggingFace Whisper WebSocket 服务器: ws://{host}:{port}")
