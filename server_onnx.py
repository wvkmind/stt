...
def init_model():
    """初始化 SenseVoice ONNX 模型"""
    global MODEL, CC
    
    if not HAS_ONNX:
        logger.error("未安装 funasr-onnx，请运行: pip install funasr-onnx")
        raise ImportError("funasr-onnx not installed")
    
    logger.info("正在加载 SenseVoice ONNX 模型...")
    logger.info("这是 C++ 加速版本，速度更快！")
...
async def handle_streaming_client(websocket, path):
    ...
    try:
        await websocket.send(json.dumps({
            "type": "connected",
            "message": "已连接到 SenseVoice ONNX STT 服务 (C++ 加速)",
            "mode": "streaming"
        }))
...
async def main():
    ...
    logger.info(f"启动 SenseVoice ONNX WebSocket 服务器: ws://{host}:{port}")
    logger.info("C++ 加速版本，速度提升 3-5 倍！")
